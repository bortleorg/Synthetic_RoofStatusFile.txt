...
def test_stale_image_action_defaults_to_keep_for_old_settings_files(settings_app):
    """Settings saved before the option existed keep reporting the model's view."""
    with open(srs.SETTINGS_FILE, "w", encoding="utf-8") as handle:
...
def test_unrecognised_stale_image_action_falls_back_to_the_default(settings_app):
    with open(srs.SETTINGS_FILE, "w", encoding="utf-8") as handle:
    ...
    assert settings_app.stale_image_action.get() == srs.DEFAULT_STALE_ACTION
